...
import cn.edu.s07150833gdmec.myguard.HomeActivity;
import cn.edu.s07150833gdmec.myguard.R;
import cn.edu.s07150833gdmec.myguard.m1home.entity.VersionEntity;

/**更新提醒工具类*/
public class VersionUpdateUtils {
    private static final int MESSAGE_NET_EEOR = 101;
    private static final int MESSAGE_I0_EE0R = 102;
    private static final int MESSAGE_JSON_EE0R = 103;
    private static final int MESSAGE_SH0EW_DIALOG = 104;
        ...
        try {
            ...
            if (execute.getStatusLine().getStatusCode() == 200) {
                ...
                if (!mVersion.equals(versionEntity.versioncode)) {
                    //版本号不一致
                    handler.sendEmptyMessage(MESSAGE_SH0EW_DIALOG);
                }
            }
            ...
            handler.sendEmptyMessage(MESSAGE_I0_EE0R);
            e.printStackTrace();
        } catch (JSONException e) {
            handler.sendEmptyMessage(MESSAGE_JSON_EE0R);
            e.printStackTrace();
        }
    }

    /**
     * @param versionEntity *弹出更新提示对话框
     */
    private void showUpdateDialog(final VersionEntity versionEntity) {
        // 创建dialog
        AlertDialog.Builder builder = new AlertDialog.Builder(context);
        builder.setTitle("检查到新版本：" + versionEntity.versioncode);// 设置标:题
        builder.setMessage(versionEntity.description);//根据服务器返回描述，设置升级描述信，息
        builder.setCancelable(false);//设置不能点击手机返回按纽隐藏对话框
        builder.setIcon(R.mipmap.ic_launcher);// 设置对话框厨标：
        //设置立即升级按钮点击事件
